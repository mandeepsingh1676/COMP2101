...
Host=$(hostname)
LAN=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
LANHOST=$(cat /etc/hosts  | grep $LAN | awk '{print $2}')
...
echo "Router Hostname : $RouterName"

check=$(cat /etc/hosts | grep $RouterAddress | wc -l)
if [[ $check -ge 1 ]];then
        echo "Already entered the Value of Router in Host file"
else
        echo "$RouterAddress $RouterName" >> /etc/hosts
fi
