...
sleepTime=$2 # delay used by sleeptime
numberOfSleeps=$2 # how many sleeps to wait for before quitting for inactivity
...
while [ $# -gt 0 ]; do
    case $1 in
        -w | --waittime )
            shift
            sleepTime="$1"
            trap 'echo "Cant terminate in between. Counting from first";doCountdown|dialog --gauge "Remaining Time" 7 60;echo "Got a Termination Again. Exiting thw Program";exit 1' 2
            ;;
        -n | --waitcount)
            shift
            numberOfSleeps="$1"
...
if [[ ! $numberOfSleeps -gt 0 ]]; then
    error_exit "$numberOfSleeps is not a valid count of sleeps to wait for signals"
fi

if [[ ! $sleepTime -gt 0 ]]; then
    error_exit "$sleepTime is not a valid time to sleep while waiting for signals"
fi
